...
const permit_js_1 = require("./permit/permit.js");
const store_js_1 = require("./permit/store.js");
const validation_js_1 = require("./validation.js");
const store_js_2 = require("./store.js");
const encrypt_js_1 = require("./encrypt.js");
const init_js_1 = require("./init.js");
const permit_z_js_1 = require("./permit/permit.z.js");
const types_js_1 = require("../types/types.js");
const encryptable_js_1 = require("../types/encryptable.js");
...
const initialize = async (params) => {
    ...
    await (0, init_js_1.InitFhevm)().catch((err) => {
        if (params.ignoreErrors) {
            ...
            return (0, result_js_1.ResultErr)(`initialize :: failed to initialize fhenixjs - is the network FHE-enabled? ${err}`);
        }
    });
    if (params.provider == null)
        return (0, result_js_1.ResultErr)("initialize :: missing provider - Please provide an AbstractProvider interface");
    if (params.securityZones != null && params.securityZones.length === 0)
        return (0, result_js_1.ResultErr)("initialize :: a list of securityZones was provided, but it is empty");
    await (0, store_js_2._store_initialize)(params);
    // `generatePermit` must set to `false` to early exit here
    if (params.generatePermit === false)
        return (0, result_js_1.ResultOk)(undefined);
...
const _checkInitialized = (state, options) => {
    if (options?.fheKeys !== false && !state.fheKeysInitialized) {
        return (0, result_js_1.ResultErr)("fhenixsdk not initialized. Use `fhenixsdk.initialize(...)`.");
    }
    if (options?.provider !== false && !state.providerInitialized)
        return (0, result_js_1.ResultErr)("fhenixsdk not initialized with valid provider. Use `fhenixsdk.initialize(...)` with a valid provider that satisfies `AbstractProvider`.");
    if (options?.signer !== false && !state.signerInitialized)
        return (0, result_js_1.ResultErr)("fhenixsdk not initialized with a valid signer. Use `fhenixsdk.initialize(...)` with a valid signer that satisfies `AbstractSigner`.");
    return (0, result_js_1.ResultOk)(null);
};
// Permit
/**
...
const createPermit = async (options) => {
    const state = store_js_2._sdkStore.getState();
    const initialized = _checkInitialized(state);
    if (!initialized.success)
        return (0, result_js_1.ResultErr)(`${createPermit.name} :: ${initialized.error}`);
    ...
    try {
        ...
        return (0, result_js_1.ResultErr)(`${createPermit.name} :: ${e}`);
    }
    store_js_1.permitStore.setPermit(state.account, permit);
    store_js_1.permitStore.setActivePermitHash(state.account, permit.getHash());
    return (0, result_js_1.ResultOk)(permit);
};
/**
...
const importPermit = async (imported) => {
    ...
    if (typeof imported === "string") {
        try {
            ...
            return (0, result_js_1.ResultErr)(`importPermit :: json parsing failed - ${e}`);
        }
    }
    ...
    if (!success) {
        const errorString = Object.entries(permitParsingError.flatten().fieldErrors)
            ...
            .join("\n");
        return (0, result_js_1.ResultErr)(`importPermit :: invalid permit data - ${errorString}`);
    }
    if (parsedPermit.type !== "self") {
        if (parsedPermit.issuer === state.account)
            ...
            return (0, result_js_1.ResultErr)(`importPermit :: invalid Permit - connected account <${state.account}> is not issuer or recipient`);
        }
    }
    let permit;
    try {
        ...
        return (0, result_js_1.ResultErr)(`importPermit :: ${e}`);
    }
    const { valid, error } = permit.isValid();
    if (!valid) {
        return (0, result_js_1.ResultErr)(`importPermit :: newly imported permit is invalid - ${error}`);
    }
    store_js_1.permitStore.setPermit(state.account, permit);
    store_js_1.permitStore.setActivePermitHash(state.account, permit.getHash());
    return (0, result_js_1.ResultOk)(permit);
...
const selectActivePermit = (hash) => {
    ...
    const initialized = _checkInitialized(state);
    if (!initialized.success)
        return (0, result_js_1.ResultErr)(`${selectActivePermit.name} :: ${initialized.error}`);
    const permit = store_js_1.permitStore.getPermit(state.account, hash);
    if (permit == null)
        return (0, result_js_1.ResultErr)(`${selectActivePermit.name} :: Permit with hash <${hash}> not found`);
    store_js_1.permitStore.setActivePermitHash(state.account, permit.getHash());
    return (0, result_js_1.ResultOk)(permit);
};
/**
...
const getPermit = (hash) => {
    ...
    const initialized = _checkInitialized(state);
    if (!initialized.success)
        return (0, result_js_1.ResultErr)(`${getPermit.name} :: ${initialized.error}`);
    if (hash == null) {
        const permit = store_js_1.permitStore.getActivePermit(state.account);
        if (permit == null)
            return (0, result_js_1.ResultErr)(`getPermit :: active permit not found`);
        return (0, result_js_1.ResultOk)(permit);
    }
    const permit = store_js_1.permitStore.getPermit(state.account, hash);
    if (permit == null)
        return (0, result_js_1.ResultErr)(`getPermit :: permit with hash <${hash}> not found`);
    return (0, result_js_1.ResultOk)(permit);
};
/**
...
const getPermission = (hash) => {
    ...
    if (!permitResult.success)
        return (0, result_js_1.ResultErr)(`${getPermission.name} :: ${permitResult.error}`);
    return (0, result_js_1.ResultOk)(permitResult.data.getPermission());
};
/**
...
const getAllPermits = () => {
    ...
    const initialized = _checkInitialized(state);
    if (!initialized.success)
        return (0, result_js_1.ResultErr)(`${getAllPermits.name} :: ${initialized.error}`);
    return (0, result_js_1.ResultOk)(store_js_1.permitStore.getPermits(state.account));
};
async function encrypt(item) {
    const state = store_js_2._sdkStore.getState();
    if (!state.coFhe.enabled)
        return (0, result_js_1.ResultErr)("coFheEncrypt :: fhenixsdk not initialized to interact with CoFHE. Set `isCoFHE: true` in `fhenixsdk.initialize`.");
    ...
    if (!initialized.success)
        return (0, result_js_1.ResultErr)(`${encrypt.name} :: ${initialized.error}`);
    // Permission
    if (item === "permission") {
        return getPermission();
    }
    // EncryptableItem
    if ((0, encryptable_js_1.isEncryptableItem)(item)) {
        ...
        try {
            ...
            return (0, result_js_1.ResultErr)(`coFheEncrypt :: tfhe_encrypt_xxxx :: ${e}`);
        }
    ...
    if (typeof item === "object" && item !== null) {
        if (Array.isArray(item)) {
            ...
            return (0, result_js_1.ResultOk)(nestedItems.map((nestedItem) => nestedItem.data));
        }
        else {
            // Object - recurse
            ...
            return (0, result_js_1.ResultOk)(result);
        }
    }
    // Primitive
    return (0, result_js_1.ResultOk)(item);
}
// Unseal
/**
...
const unsealCiphertext = (ciphertext, account, hash) => {
    ...
    const resolvedHash = hash ?? store_js_1.permitStore.getActivePermitHash(resolvedAccount);
    if (resolvedAccount == null || resolvedHash == null) {
        return (0, result_js_1.ResultErr)(`unsealCiphertext :: PermitV2 hash not provided and active PermitV2 not found`);
    }
    const permit = store_js_1.permitStore.getPermit(resolvedAccount, resolvedHash);
    if (permit == null) {
        return (0, result_js_1.ResultErr)(`unsealCiphertext :: PermitV2 with account <${account}> and hash <${hash}> not found`);
    }
    let unsealed;
    try {
        ...
        return (0, result_js_1.ResultErr)(`unsealCiphertext :: ${e}`);
    }
    return (0, result_js_1.ResultOk)(unsealed);
};
/**
...
function unseal(item, account, hash) {
    const resolvedAccount = account ?? store_js_2._sdkStore.getState().account;
    const resolvedHash = hash ?? store_js_1.permitStore.getActivePermitHash(resolvedAccount);
    if (resolvedAccount == null || resolvedHash == null) {
        return (0, result_js_1.ResultErr)(`unseal :: PermitV2 hash not provided and active PermitV2 not found`);
    }
    const permit = store_js_1.permitStore.getPermit(resolvedAccount, resolvedHash);
    if (permit == null) {
        return (0, result_js_1.ResultErr)(`unseal :: PermitV2 with account <${account}> and hash <${hash}> not found`);
    }
    let unsealed;
    try {
        ...
        return (0, result_js_1.ResultErr)(`unseal :: ${e}`);
    }
    return (0, result_js_1.ResultOk)(unsealed);
}
// Export
exports.fhenixsdk = {
    store: store_js_2._sdkStore,
    initialize,
    createPermit,
    importPermit,
    selectActivePermit,
