...
function getAsSealedItem(value) {
    if (Array.isArray(value) &&
        ...
        types_1.FheAllUTypes.includes(parseInt(`${value[1]}`)))
        return {
            data: value[0],
            utype: value[1],
        };
...
function isSealedBool(value) {
    return parseInt(`${value.utype}`) === types_1.FheUType.bool;
}
exports.isSealedBool = isSealedBool;
function isSealedUint(value) {
    return types_1.FheUintUTypes.includes(parseInt(`${value.utype}`));
}
exports.isSealedUint = isSealedUint;
function isSealedAddress(value) {
    return parseInt(`${value.utype}`) === types_1.FheUType.address;
}
