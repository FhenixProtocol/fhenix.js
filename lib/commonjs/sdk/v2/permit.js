...
const permit_generate_1 = require("./permit.generate");
...
class PermitV2 {
    constructor(options, metadata) {
        ...
        this.getSignatureParams = (chainId, primaryType) => {
            return {
                domain: (0, permit_generate_1.getSignatureDomain)(chainId),
                ...(0, permit_generate_1.getSignatureTypesAndMessage)(primaryType, permit_generate_1.SignatureTypes[primaryType], this.getPermission(true)),
            };
        };
        /**
        ...
        this.unsealCiphertext = (ciphertext) => {
            ...
            if ((0, utils_1.chainIsHardhat)(this._signedChainId))
                return (0, utils_1.hardhatMockUnseal)(ciphertext);
    ...
    unseal(item) {
        // SealedItem
        if ((0, types_1.isSealedItem)(item)) {
            const bn = (0, utils_1.chainIsHardhat)(this._signedChainId)
                ? (0, utils_1.hardhatMockUnseal)(item.data)
                : this.sealingPair.unseal(item.data);
            if ((0, types_1.isSealedBool)(item)) {
                // Return a boolean for SealedBool
                return Boolean(bn).valueOf();
            }
            if ((0, types_1.isSealedAddress)(item)) {
                // Return a string for SealedAddress
                return (0, ethers_1.getAddress)(`0x${bn.toString(16).slice(-40)}`);
            }
            if ((0, types_1.isSealedUint)(item)) {
