...
export const _store_initialize = async (params) => {
    ...
    if (!chainIsHardhat(chainId) && !_sdkStore.getState().fheKeysInitialized) {
        await Promise.all(securityZones.map((securityZone) => _store_fetchFheKey(chainId, provider, securityZone, true)));
    }
 ...
 */
export const _store_fetchFheKey = async (chainId, provider, securityZone = 0, forceFetch = false) => {
