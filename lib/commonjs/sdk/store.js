...
exports._store_fetchFheKey = exports._store_initialize = exports._store_setFheKey = exports._store_getConnectedChainFheKey = exports._store_getFheKey = exports._store_chainId = exports._sdkStore = void 0;
...
const _store_chainId = () => exports._sdkStore.getState().chainId;
exports._store_chainId = _store_chainId;
...
const _store_initialize = async (params) => {
    const { provider, signer, securityZones = [0], contracts: contractRequirements = [], projects: projectRequirements = [], isCoFhe = false, coFheUrl = undefined, } = params;
    ...
    const coFheEnabledChanged = isCoFhe !== exports._sdkStore.getState().coFhe.enabled;
    ...
    if (!(0, utils_1.chainIsHardhat)(chainId) && !exports._sdkStore.getState().fheKeysInitialized) {
        await Promise.all(securityZones.map((securityZone) => (0, exports._store_fetchFheKey)(chainId, provider, securityZone, true)));
    }
 ...
 */
const _store_fetchFheKey = async (chainId, provider, securityZone = 0, forceFetch = false) => {
    const storedKey = (0, exports._store_getFheKey)(chainId, securityZone);
    if (storedKey != null && !forceFetch)
        return storedKey;
    const coFhe = exports._sdkStore.getState().coFhe;
    ...
    if (!coFhe.enabled) {
        ...
        publicKey = await provider.call({ to: consts_1.FheOpsAddress, data: callData });
    }
    ...
    if (publicKey == null || typeof publicKey !== "string") {
        throw new Error(`Error initializing fhenixjs; FHE publicKey fetched from ${coFhe.enabled ? "CoFHE" : "Provider"} invalid: not a string`);
    }
    if (publicKey === "0x") {
    ...
    if (publicKey.length < consts_1.PUBLIC_KEY_LENGTH_MIN) {
        throw new Error(`Error initializing fhenixjs; got shorter than expected FHE publicKey: ${publicKey.length}. Expected length >= ${consts_1.PUBLIC_KEY_LENGTH_MIN}`);
    }
