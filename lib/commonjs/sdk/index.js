...
class FhenixClientBase {
    ...
    constructor(params) {
        ...
        this.cofheURL = params.cofheURL ? params.cofheURL : consts_js_1.DEFAULT_COFHE_URL;
        if (!this.provider) {
    ...
    async generatePermit(contractAddress, provider, signer) {
        ...
        const permit = await (0, index_js_1.generatePermit)(contractAddress, provider || this.provider, signer);
    ...
    storePermit(permit, account) {
        ...
        if (typeof permit.sealingKey.unseal == "undefined") {
            this.permits[permit.contractAddress] = (typeof permit == "string") ? (0, index_js_1.parsePermit)(permit) : (0, index_js_1.parsePermit)(JSON.stringify(permit));
        }
        else {
    ...
    static async getFheKeyFromFHEOS(securityZone = 0, cofheURL) {
        ...
        const buff = (0, utils_js_1.fromHexString)(publicKey);
        try {
            return fhe_js_1.TfheCompactPublicKey.deserialize(buff);
        }
        catch (err) {
...
class FhenixClientSync extends FhenixClientBase {
    ...
    static async create(params) {
        ...
        const fhePublicKeys = await Promise.all(securityZones.map((securityZone) => (params.cofhe == true) ? FhenixClientBase.getFheKeyFromFHEOS(securityZone, params.cofheURL ? params.cofheURL : consts_js_1.DEFAULT_COFHE_URL) : FhenixClientBase.getFheKeyFromProvider(provider, securityZone)));
